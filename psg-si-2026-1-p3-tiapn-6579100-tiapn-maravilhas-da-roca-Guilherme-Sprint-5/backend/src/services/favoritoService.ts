 ...
 async getFavoritosByUser(UsuCodigo: number) {
    const [rows] = await pool.execute(
      ...
      SELECT 
        ...
        p.ProNome,
        p.ProPreco,
        p.ProDesc AS ProDescricao,  -- 👈 CORRIGIDO: Usa o nome real do banco 'ProDesc' e mapeia para o frontend
        p.ProImagem AS ProImagem       -- 👈 Mantido conforme o banco de dados
      FROM Favorito f
