<template>
  <div class="min-h-screen bg-gradient-to-br from-[#FAF6EE] via-[#FDFBF7] to-[#F3EAD9] text-[#4A3728] antialiased">
    ...
    <main class="max-w-7xl mx-auto px-4 pb-20 pt-10">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-12">
        ...
        >
          ...
          <RouterLink :to="`/produto/${item.ProCodigo}`" class="block cursor-pointer">
            <div class="relative overflow-hidden aspect-[4/3] sm:aspect-[1/1] md:aspect-[4/3] rounded-[2.2rem] m-3.5 bg-[#FAF6EE] border border-[#EED9C4]/15 shadow-inner flex items-center justify-center">
                <img 
                  v-if="item.ProImagem || item.ProFoto" 
                  :src="`/images/${item.ProImagem || item.ProFoto}`" 
                  :alt="item.ProNome"
                  class="w-full h-full object-cover group-hover:scale-105 transition duration-700"
                />
              <span v-else class="text-7xl group-hover:scale-110 transition duration-700">
                🧀
              </span>
            </div>
          </RouterLink>
          ...
          <button
            @click="removerFavorito(item.FavCodigo)"
            class="absolute top-6 right-6 bg-white/90 hover:bg-white text-gray-400 hover:text-red-500 p-2 rounded-full shadow-sm transition-all border border-gray-100 z-10"
          >
          ...
          <div class="p-6 pt-3 flex flex-col flex-grow justify-between text-center">
            ...
            <RouterLink :to="`/produto/${item.ProCodigo}`" class="block cursor-pointer hover:opacity-80">
            ...
            <p class="text-[#7A5C43] text-sm font-light mt-2 mb-6 line-clamp-2 min-h-[2.5rem] opacity-90">
              {{ item.ProDescricao || 'Produto selecionado diretamente da roça.' }}
            </p>
...
const removerFavorito = async (favCodigo) => {
  if (!confirm('Deseja remover este item dos seus favoritos?')) return

  try {
    const token = localStorage.getItem('token')

    const res = await fetch(`${API_URL}/${favCodigo}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`
      }
    ...
    if (res.ok) {
      favoritos.value = favoritos.value.filter(i => i.FavCodigo !== favCodigo)
    } else {
