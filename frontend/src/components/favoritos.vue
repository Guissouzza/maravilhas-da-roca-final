    ...
    <div class="max-w-[1200px] mx-auto px-6">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        >
          ...
          <button class="absolute top-3 right-3 bg-white/90 hover:bg-white text-gray-400 hover:text-red-500 p-2 rounded-full shadow-sm transition-all border border-gray-100">
...
const buscarFavoritos = async () => {
  try {
    const resposta = await fetch('http://localhost:3000/api/favoritos/1');
    if (resposta.ok) {
