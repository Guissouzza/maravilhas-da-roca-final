...
/* CACHE GLOBAL (não reseta ao trocar rota) */
let cachedCartCount = 0;
let cachedFavoritesCount = 0;
...
/* inicia usando cache */
const cartCount = ref(cachedCartCount);
const favoritesCount = ref(cachedFavoritesCount);
const userEmail = ref<string | null>(cachedUserEmail);
...
const loadCart = async () => {
  try {
    ...
    const total = (response.data.items || []).reduce(
      (t: number, i: any) =>
        t + Number(i.Quantidade || 0),
      0
    );

    cartCount.value = total;
    cachedCartCount = total;
  } catch {
    cartCount.value = cachedCartCount;
  }
};
...
const loadFavorites = async () => {
  try {
    ...
    favoritesCount.value = total;
    cachedFavoritesCount = total;
  } catch {
    favoritesCount.value = cachedFavoritesCount;
  }
};
  ...
  >
    <div class="max-w-7xl mx-auto flex items-center gap-6">
      ...
      <div class="flex items-center gap-5 ml-auto">
        ...
        <!-- NAV -->
        <nav
          class="hidden lg:flex gap-6 text-sm font-bold text-[#7A5C43]"
        >
          <RouterLink to="/">
            Início
          </RouterLink>
        ...
        >
          ...
          <span
            class="absolute -top-2 -right-2 bg-[#A0522D] text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full"
          >
            {{ favoritesCount }}
          </span>
        </RouterLink>
        ...
        <div class="relative">
          ...
          >
            <div
              class="px-4 py-3 border-b text-xs text-[#7A5C43]"
            >
              <p
                class="font-bold text-[#362212]"
              >
                Logado como:
              </p>
    ...
    >
      <nav
        class="flex flex-col gap-3 font-bold text-[#7A5C43]"
      >
        <RouterLink
